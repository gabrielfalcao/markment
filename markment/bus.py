...
class Function(object):
    ...
    def as_string(self, **kwargs):
        ...
        return 'Function({0})'.format(", ".join([itemize(kw), itemize(kwargs)]))

    def __str__(self):
        if self.parent:
            return self.as_string(parent=self.parent)
        else:
...
class Speaker(object):
    def __init__(self, name, actions, output=None):
        ...
        if not isinstance(actions, list):
            raise TypeError('actions must be a list of strings. %r' % actions)

        for action in map(underlinefy, actions):
            self.actions[action] = partial(self.for_decorator, action)
            self.actions[action].shout = partial(self.shout, action)
    ...
    def for_decorator(self, action, callback):
        ...
        responder = Function(callback, parent=self)
